...
from CausalPerturb.modelDL import CAPE
from CausalPerturb.dataDL import load_dataset_splits
from CausalPerturb.plotting import plot_loss
from CausalPerturb.plotting import plot_metric
...
def find_best_k(oNMFres, dataset_path, alpha=1, SS_weight=0.3):
    ...
    # Set all powers and scaling parameters
...
def cape_run(data_path, dataset_name, model_index=0, perturbation_key='condition', split_key=None,
    ...
    The embeddings (basal state, factor-level expression), the orthogonal loading matrix,
     the model file (.pt file), the loss and metric curves are stored in dataset_name/train_res.
    Parameters
    ----------
    data_path: string
    ...
    if not os.path.exists(dataset_name):
        os.makedirs(dataset_name)  # makedir
    dataset_path = os.path.join(dataset_name, 'train_res')
    if not os.path.exists(dataset_path):
    ...
    if len(perts_name_list) <= 20:
        ...
        for pert in perts_name_list:
            res = scib.metrics.metrics(adata=basal_adata[basal_adata.obs.condition.isin([pert, 'control'])],
                                       ...
                                       batch_key='condition', label_key='cell_type', silhouette_=True,
                                       verbose=True).loc[
                'ASW_label/batch'][0]
...
def model_train(data_path, dataset_name, perturbation_key='condition', split_key=None,
    ...
    The embeddings (basal state, factor-level expression), the orthogonal loading matrix,
     the model file (.pt file), the loss and metric curves of the best model are stored in dataset_name/train_res.
    Parameters
    ----------
    data_path: string
    The path where the dataset locates
    ...
    The weight of adversarial loss, default [0.5, 1, 2]
    ...
    if np.max(np.array(asw1_res)) < 0.83 or np.max(np.array(asw2_res)) < 0.83:
        print(
            'Warning: The mixing performance is not good. Try higher lambda_adv values in lambda_adv_list, or set a lower gradient penalty' +
            ' for the discriminator by using hparams={\'penalty_adversary\':val}, val<10')
    ...
    if best_model_index is None:
        topmodels_num = min(4, len(asw_agg_res))
...
def CF_single_target_single_factor(target, factor, basal, treated,  pert_key='condition', n_estimators=2000,
    ...
    if "control" in basal.obs:
        pass
    else:
        ctrl_dummy = np.zeros_like(basal.obs[pert_key])
        for i in range(len(ctrl_dummy)):
            if basal.obs[pert_key][i] == 'control':
                ctrl_dummy[i] = 1
        basal.obs['control'] = ctrl_dummy.tolist()
    T = 1 - basal[basal.obs[pert_key].isin([target, 'control'])].obs.control
...
def CF_single_target_all_factor(target, basal, treated,  pert_key='condition', n_estimators=2000,
                                min_samples_leaf=5, one_side=False, random_state=0, alpha=0.05):
    """
...
def CF_all_target_all_factor(dataset_name, basal, treated, pert_key='condition', return_df=True, n_estimators=2000,
    ...
    The column name of the perturbation labels in AnnData, default  'condition'
    return_df: bool
    Return results in csv files or not, default True
    n_estimators: int
    Number of trees
    min_samples_leaf: int
    The minimum number of samples required to be at a leaf node.
    ...
    sig_factor_all_dict = {}
    for pert in basal.obs[pert_key].cat.categories:
        if pert != 'control':
    ...
    if return_df:
        for pert in basal.obs[pert_key].cat.categories:
            if pert != 'control':
                cell_names = basal[basal.obs[pert_key].isin([pert, 'control'])].obs_names
                tau_factor_mean = tau_factor_mean_all_dict[pert]
                column_names = ['factor_'+str(i+1) for i in range(tau_factor_mean.shape[1])]
                tau_factor_mean_df = pd.DataFrame(tau_factor_mean, index=cell_names, columns=column_names)
                tau_factor_mean_df.to_csv(os.path.join(dataset_path, '{}_tau_factor_mean_df.csv'.format(pert)))
                tau_q_val_factor = tau_q_val_factor_all_dict[pert]
                tau_q_val_factor_df = pd.DataFrame(tau_q_val_factor, index=cell_names, columns=column_names)
                tau_q_val_factor_df.to_csv(os.path.join(dataset_path, '{}_tau_q_val_factor_df.csv'.format(pert)))
                sig_factor = sig_factor_all_dict[pert]
                sig_factor_df = pd.DataFrame(sig_factor, index=cell_names, columns=column_names)
                sig_factor_df.to_csv(os.path.join(dataset_path, '{}_sig_factor_df.csv'.format(pert)))
    if verbose:
...
def CF_GI_single_target_single_factor(dataset_name, target, factor, basal, treated, adata,
    ...
    cell_names = (adata[adata.obs[pert_key].isin([t1, t2, target, 'control'])].obs_names).tolist()
    ...
    all_res_df = pd.DataFrame(all_res, columns=['tau_a', 'tau_b', 'tau_int', 'pval_a', 'pval_b', 'pval_int',
                                                ...
                                                'sig_epib',
                                                'sig_RD', 'sig_NE'], index=cell_names)
    if return_ctrl:
        all_res_ctrl = all_res[ctrl_idx, :]
        all_res_ctrl_df = all_res_df.iloc[ctrl_idx]
        if save:
            np.save(
