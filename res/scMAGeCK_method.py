...
ridge=sklearn.linear_model.Ridge(alpha=0.01,max_iter=10000)
ridge.fit(X,Y)
Be=pd.DataFrame(ridge.coef_)
Be.columns=X.columns
Be.index = adata_stim.var_names.values
Be.to_csv('SM2018_Tcells/stim/scmageck_beta.csv')
scmageck_pval_mtx=permute(X,Y,Be,n_permut=500)
pvals = scmageck_pval_mtx.flatten()
_, pvals_corrected, _, _ = multipletests(pvals, method="fdr_bh")
scmageck_qval_mtx = pvals_corrected.reshape(scmageck_pval_mtx.shape)
np.save("SM2018_Tcells/stim/scmageck_qval.npy",scmageck_qval_mtx)
...
Be.to_csv('SM2018_Tcells/unstim/scmageck_beta.csv')
scmageck_pval_mtx=permute(X,Y,Be,n_permut=500)
pvals = scmageck_pval_mtx.flatten()
_, pvals_corrected, _, _ = multipletests(pvals, method="fdr_bh")
scmageck_qval_mtx = pvals_corrected.reshape(scmageck_pval_mtx.shape)
np.save("SM2018_Tcells/unstim/scmageck_qval.npy",scmageck_qval_mtx)
