...
treated=sc.read_h5ad(os.path.join(dataset_name,'CAPE','model_index=0_treated.h5ad')) # outcome factor state
...
```python
gene_loading=np.load(os.path.join(dataset_name,'CAPE','model_index=0_gene_loading.npy')) # gene loading matrix
...
Check the basic usage with a toy data ('toydata.py') in 'res'.
